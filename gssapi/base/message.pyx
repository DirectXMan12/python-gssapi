...
def getMIC(SecurityContext context not None, message, qop=None):
    ...
    cdef gss_buffer_desc token_buffer = gss_buffer_desc(0, NULL)  # GSS_C_EMPYT_BUFFER
...
def verifyMIC(SecurityContext context not None, message, token, return_bool=False):
    """
    verifyMIC(context, message, token, return_bool=False) -> int or (bool, int, int, int)
...
def wrapSizeLimit(SecurityContext context not None, OM_uint32 output_size, confidential=True, qop=None):
    """
...
def wrap(SecurityContext context not None, message, confidential=True, qop=None):
    """
